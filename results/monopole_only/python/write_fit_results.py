...
def load_fit_results(kind, sample, kmin, z_weighted, p):
    ...
    if kind == 'Mocks':
        d = load_ezmock_results('v1.8e-fph', str(sample),
                                kmin+'-0.3', 'basemodel-N-fnl', z_weighted, p=p, ells=[0])
    else:
        if sample == 'N+S':
            r = load_joint_data_results(kmin, z_weighted, p, ells=[0])
            r.burnin = int(r.iterations//2)
        else:

            d = load_data_results(
                'v1.9f', str(sample), kmin+'-0.3', 'basemodel-N-fnl', '0.8-2.2', z_weighted, p=p, ells=[0])
            d.set_fit_results()
            r = d.results
            r.burnin = int(r.iterations//2)
